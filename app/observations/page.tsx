...
import { getTranche4ProductionObservationPageIfActive } from "@/src/server/tranche4/production-model";
...
export default async function ObservationsPage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string }> }) {
  ...
  const tranche4 = await getTranche4ProductionObservationPageIfActive(Number.isSafeInteger(page) ? page : 1, params.q ?? "");
  if (tranche4) return <AppShell><HeroSection route={route} /><CandidateObservationExplorer result={tranche4.result} /></AppShell>;
  let releaseId;
  let records;
  try {
