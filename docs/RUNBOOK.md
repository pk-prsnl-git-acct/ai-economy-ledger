...
1. Worker upload, runtime variables/secrets, and zone route `aieconomyledger.com/* -> ai-economy-ledger` succeeded.
2. Hosted Supabase Data API was configured to expose the intended `api` schema while leaving `ledger` and `private` unexposed.
3. Public snapshot RPC calls were fixed to send PostgREST `accept-profile: api` and `content-profile: api` headers.
4. Cloudflare account workers.dev subdomain `aieconomyledger.workers.dev` was initialized because Cron schedules require an account workers.dev subdomain.
