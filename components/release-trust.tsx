...
export function ReleaseDetail({ manifest, records, sources, revisionCount, correctionCount }: { manifest: ReleaseManifest; records: PublicRecord[]; sources: SourceManifestEntry[]; revisionCount: number; correctionCount: number }) {
  ...
  const layers = summarizeAiStack(records);
  const downloadableArtifacts = manifest.artifacts.filter((artifact) => artifact.mediaType.startsWith("application/json") || artifact.mediaType.startsWith("text/csv"));
  return <>
    ...
    <section className="panel artifact-panel"><div><p className="panel-label">Artifact downloads</p><h2>Exact public release artifacts</h2><p>Machine-readable downloads retain exact values. Display formatting is applied only in the interface.</p></div><div className="artifact-links">{downloadableArtifacts.map((artifact) => <DownloadLink href={`${releasePath}/artifacts/${encodeURIComponent(artifact.name)}`} key={artifact.name}>{artifact.name}</DownloadLink>)}</div></section>
